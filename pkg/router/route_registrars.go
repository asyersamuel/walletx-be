...
// RegisterTelegramRoutes registers the Telegram webhook and email-verification
...
func RegisterTelegramRoutes(r *gin.RouterGroup, telegramHandler *handlers.TelegramHandler) {
	tg := r.Group("/telegram")
	{
		tg.POST("/webhook", telegramHandler.HandleWebhook)
		tg.GET("/verify", telegramHandler.VerifyToken)
	}
}
