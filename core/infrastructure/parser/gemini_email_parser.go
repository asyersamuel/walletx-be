...
func NewGeminiEmailParser(logger ports.Logger, apiKey string) parser.EmailParser {
...
func (p *geminiEmailParser) Parse(rawBody string) (*parser.ParsedTransactionData, error) {
	if rawBody == "" {
	...
	return &parser.ParsedTransactionData{
		Amount:          amount,
		Merchant:        merchant,
...
func (p *geminiEmailParser) extractDate(text string) *time.Time {
	datePatterns := []struct {
		...
		{
			re:     regexp.MustCompile(`(?i)\b(\d{2}/\d{2}/\d{4}\s+\d{2}:\d{2}:\d{2})\b`),
			layout: "02/01/2006 15:04:05",
		},
		{
			re:     regexp.MustCompile(`(?i)\b(\d{2}\s+[A-Za-z]{3,}\s+\d{4}\s+\d{2}:\d{2}:\d{2})\b`),
